...
function Table() {
  ...
  function data() {
    ...
    axios.get(`http://localhost:9009/book`)
      .then(res => {
  ...
  function deleteuser(id) {
    // let token = localStorage.getItem('token')
    console.log(id);
    axios.delete(`http://localhost:9009/book/${id}`)
      .then((result) => {
  ...
  function updateuser(id) {
    // let token = localStorage.getItem('token')
    console.log(id);
    axios.put(`http://localhost:9009/e/${id}`)
      .then((result) => {
        console.log("result.data", result.data);
        history.push("/edit")
        data()
        ...
        <Grid container spacing={3}>
          {user.map(user => (
            <Grid item key={user.id} xs={12} md={6} lg={4}>
              <NoteCard note={user} handleclick={deleteuser} addcart={addCart} update={updateuser} />
            </Grid>
          ))}
        </Grid>
      </Container>
