...
function Edit() {
    ...
    const getuser = () => {
        ...
        // } else {      
            ...
            axios.get(`http://localhost:9009/Abc`, { headers: { 'x-access-token': token } }).then((result) => {
                console.log("result.data", result)
                if (result.data.success === true) {
                    setUsername(result.data.data[0].username)
                    setdescription(result.data.data[0].description)
                    setquantities(result.data.data[0].quantities)
                    setprice(result.data.data[0].price)
                    // setProfile(result.data.profile_url)
                }
                 else {
                    return;
                }
    ...
    const handleClick = () => {
        ...
        FD.append('profile_file',profile[0])
          ...
          let token = localStorage.getItem("token");
        axios.put(`http://localhost:9009/e/${id}`, FD,{ headers: { 'x-access-token': token } })
        history.push('/Table')
        setOpen(true);
    };
